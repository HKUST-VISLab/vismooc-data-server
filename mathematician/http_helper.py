...
def download_single_file(url, save_dir=None, headers=None, params=None, common_suffix='',
                         md5_checksum=None, retry_times=5, delay=1):
...
def download_multi_files(urls, save_dir, headers=None, common_suffix='', md5_checksums=None,
                         retry_times=5, delay=1, process_pool_size=(os.cpu_count() or 1)):
...
class HttpConnection:
    ...
    def get(self, url, params=None, retry_times=5, delay=1):
        '''The http GET method
        '''
        response = get(self.__host + url, self.headers, params, retry_times, delay)
    ...
    def post(self, url, params, retry_times=5, delay=1):
        ...
        '''
        response = post(self.__host + url, self.headers, params, retry_times, delay)
        if response is not None:
            if response.get_headers().get("Set-Cookie") is not None:
                self.__headers["Cookie"] = response.get_headers().get("Set-Cookie")
        else:
    ...
    def download_file(self, url, save_dir, common_suffix='', md5_checksum=None, retry_times=5,
        ...
        return download_single_file(self.__host + url, save_dir, self.__headers, None,
                                    common_suffix, md5_checksum, retry_times, delay)

    def download_files(self, urls, save_dir, common_suffix='', md5_checksums=None, retry_times=5,
                       delay=1):
        '''Download a set of files
        '''
        return download_multi_files([self.__host + url for url in urls], save_dir, self.__headers,
                                    common_suffix, md5_checksums, retry_times, delay)
